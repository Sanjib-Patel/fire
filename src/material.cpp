...
namespace material {
		...
		const int bp, const int bn, const int bb, const int bbl, const int bbd, const int br, const int bq)
	{
		...
		if (const auto b_minor_pieces = bb + bn; w_minor_pieces > b_minor_pieces)
		{
			...
			if (w_minor_pieces > b_minor_pieces + 2) score += up_two_pieces_bonus;
			// white bishop bonus 
			if (wb > bb) score += more_bishops_bonus;
			// white knight bonus 				
			if (wn > bn) score -= more_knights_bonus;
		}
		else if (w_minor_pieces < b_minor_pieces)
		{
			// black minor piece bonus			
			if (b_minor_pieces > w_minor_pieces + 2)score -= up_two_pieces_bonus;
			// black bishop bonus 
			if (wb < bb)score -= more_bishops_bonus;
			// black knight bonus 				
			if (wn < bn)score += more_knights_bonus;
