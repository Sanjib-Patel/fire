...
namespace search {
	...
	int alpha_beta(position& pos, int alpha, int beta, int depth, bool cut_node)
	{
		...
		if (my_thread == thread_pool.main())
		{
			if (auto* main_thread = static_cast<mainthread*>(my_thread); ++main_thread->interrupt_counter >= 4096)
			{
				...
				{
					if (main_thread->quick_move_evaluation_stopped)	return alpha;
					if (auto elapsed = time_control.elapsed(); elapsed > 1000 || elapsed > time_control.optimum() / 16)
					{
						main_thread->quick_move_evaluation_stopped = true;	return alpha;
					}
				}
				send_time_info();
		...
		if (!root_node)
		{
			if (signals.stop_analyzing.load(std::memory_order_relaxed) || pi->move_repetition || pi->ply >= max_ply)
				return pi->ply >= max_ply && !state_check ? evaluate::eval(pos) : draw[pos.on_move()];
				...
				: hash_entry->bounds() & north_border))
		{
			if (hash_move)
			{
				if (hash_value >= beta)	update_stats(pos, state_check, hash_move, depth, nullptr, 0);
				else update_stats_minus(pos, state_check, hash_move, depth);
			}
			return hash_value;
		}
		if (!root_node && tb_number && depth >= tb_probe_depth && pos.material_or_castle_changed())
		{
			...
			{
				auto value = no_score;
				if (number_pieces <= egtb::max_pieces_wdl)	value = egtb::egtb_probe_wdl(pos);
				else if (number_pieces <= egtb::max_pieces_dtm)	value = egtb::egtb_probe_dtm(pos);
				else value = no_score;
				if (value != no_score)
				{
					hash_entry = main_hash.replace(key64);
					hash_entry->save(key64, value_to_hash(value, pi->ply), exact_value, std::min(max_depth - plies, depth + 6 * plies), no_move, no_score, main_hash.age());
					return value;
				}
			}
		}
		...
		if (hash_entry && hash_entry->eval() != no_score)
		{
			...
			if (hash_value != no_score)
				if (hash_entry->bounds() & (hash_value > eval ? south_border : north_border))eval = hash_value;
		}
		else
		{
			...
			hash_entry = main_hash.replace(key64);
			hash_entry->save(key64, no_score, no_limit + pi->strong_threat, no_depth, no_move, pi->position_value, main_hash.age());
		}
		if (pi->previous_move != null_move && (pi - 1)->position_value != no_score && pi->material_key == (pi - 1)->material_key)
		{
		...
		if (pi->no_early_pruning)goto no_early_pruning;
		if (constexpr auto razoring_min_depth = 4; !pv_node
			&& depth < razoring_min_depth * plies
			&& !hash_move
			&& eval + razor_margin <= alpha)
		{
			if (constexpr auto razoring_qs_min_depth = 2; depth < razoring_qs_min_depth * plies)return q_search<nonPV, false>(pos, alpha, beta, depth_0);
			auto r_alpha = alpha - razor_margin;
			if (auto val = q_search<nonPV, false>(pos, r_alpha, r_alpha + score_1, depth_0); val <= r_alpha)return val;
		}
			...
			&& (!thread_pool.analysis_mode || depth < null_move_max_depth * plies))
		{
			...
			r = depth < 4 * plies ? depth : (null_move_tm_base + null_move_tm_mult * (depth / plies)
				+ std::max(std::min(null_move_depth_greater_than_mult * (eval - beta)
					/ static_cast<int>(null_move_depth_greater_than_div)
			...
			pos.take_null_back();
			if (constexpr auto value_less_than_beta_margin = 100; (pi - 1)->lmr_reduction
				&& (value < beta - static_cast<int>(value_less_than_beta_margin)
					|| value < -longest_mate_score))
			{
				if ((pi - 1)->lmr_reduction <= 2 * plies)return beta - score_1;
				depth += 2 * plies;
			}
			if (value >= beta)
			{
				if (value >= longest_mate_score)value = beta;
				if (constexpr auto value_greater_than_beta_max_depth = 12; depth < value_greater_than_beta_max_depth
					* plies && abs(beta) < win_score)return value;
				...
				if (val >= beta)return value;
			}
		}
		else if (constexpr auto dummy_null_move_threat_min_depth_mult = 6; thread_pool.dummy_null_move_threat && depth
			>= dummy_null_move_threat_min_depth_mult * plies && eval >= beta && (pi - 1)->lmr_reduction)
		{
			pi->mp_end_list = (pi - 1)->mp_end_list;
			pos.play_null_move();
			(pi + 1)->no_early_pruning = true;
			...
			&& abs(beta) < longest_mate_score)
		{
			...
			while ((move = movepick::pick_move(pos)) != no_move)
			{
				if (pos.legal_move(move))
				{
					...
					if (value >= pc_beta)return value;
				}
			}
		}
		if (constexpr auto no_early_pruning_pv_node_depth_min = 5;
			depth >= (pv_node ? no_early_pruning_pv_node_depth_min * plies
				: no_early_pruning_non_pv_node_depth_min * plies)
			&& !hash_move
			&& (pv_node
				|| cut_node
				|| pi->position_value + static_cast<int>(no_early_pruning_position_value_margin) >= beta))
		{
	...
	view_all_moves:
		...
		progress = pi->position_value >= (pi - 2)->position_value || (pi - 2)->position_value == no_score;
		auto late_move_count = depth < late_move_count_max_depth * plies ? late_move_number(depth, progress) : 999;
		if (only_quiet_check_moves)	late_move_count = 1;
		...
		while ((move = movepick::pick_move(pos)) != no_move)
		{
			...
			if (move == pi->excluded_move)continue;
			if (root_node && my_thread->root_moves.find(move) < my_thread->active_pv)continue;
			...
			if (!bench_active && root_node && my_thread == thread_pool.main())
			{
				if (constexpr auto info_currmove_interval = 4000; time_control.elapsed() > info_currmove_interval)
					acout() << "info currmove " << util::move_to_string(move, pos) << " currmovenumber " << move_number
					+ my_thread->active_pv << std::endl;
			}
				...
				&& pos.legal_move(move))
			{
				...
				auto r_beta = hash_value
					- static_cast<int>(static_cast<uint32_t>(depth) / plies
						* excluded_move_r_beta_hash_value_margin_mult / excluded_move_r_beta_hash_value_margin_div);
				...
				&& pi->non_pawn_material[pos.on_move()])
			{
				...
				if (move_number >= late_move_count)	continue;
				if (constexpr auto quiet_moves_max_depth = 6; depth < quiet_moves_max_depth * plies && pi->mp_stage >= quietmoves)
				{
					auto offset = counter_move_values::calculate_offset(moved_piece, to_square(move));
					if (constexpr auto sort_cmp = static_cast<int>(sort_cmp_sort_value);
						(!cmh || cmh->value_at_offset(offset) < sort_cmp)
						&& (!fmh || fmh->value_at_offset(offset) < sort_cmp)
						&& (cmh && fmh || !fmh2 || fmh2->value_at_offset(offset) < sort_cmp))
						continue;
					if (constexpr auto quiet_moves_max_gain_mult = 12;
						pos.thread_info()->max_gain_table.get(moved_piece, move)
						< static_cast<int>(quiet_moves_max_gain_base)
						- static_cast<int>(quiet_moves_max_gain_mult)
						* static_cast<int>(static_cast<uint32_t>(depth) / plies))continue;
				}
				...
				if (predicted_depth < predicted_depth_max_depth * plies
					&& pi->position_value + futility_margin_ext(predicted_depth) <= alpha)continue;
				if (constexpr auto predicted_depth_see_test_base = 300; predicted_depth < predicted_depth_max_depth * plies
					&& !pos.see_test(move, std::min(see_0, predicted_depth_see_test_base - predicted_depth_see_test_mult
						* predicted_depth * predicted_depth / 64)))	continue;
			}
				...
				&& best_score > -longest_mate_score)
			{
				...
				if (constexpr auto non_root_node_see_test_base = 150;
					pi->mp_stage != good_captures
					&& extension != plies
					&& !pos.see_test(move, std::min(see_knight
						- see_bishop, non_root_node_see_test_base
						- non_root_node_see_test_mult * depth * depth / 64)))continue;
			}
			if (!root_node && !pos.legal_move(move))
			{
				pi->move_number = --move_number; continue;
			}
				...
				&& !capture_or_promotion)
			{
				...
				if (!pv_node && cut_node)r += 2 * plies;
				if (piece_type(moved_piece) >= pt_knight && !pos.see_test(make_move(to_square(move), from_square(move)), see_0))
					r -= 2 * plies;
			...
			if (pv_node && (move_number == 1 || value > alpha && (root_node || value < beta)))
			{
				...
				(pi + 1)->pv = pv;
				(pi + 1)->pv[0] = no_move;
				if (new_depth < plies && !(pi->ply & 1))new_depth = plies;
				value = new_depth < plies
					? gives_check
			...
			if (signals.stop_analyzing.load(std::memory_order_relaxed))	return alpha;
			if (my_thread == thread_pool.main() && static_cast<mainthread*>(my_thread)->quick_move_evaluation_stopped)return alpha;
			if (root_node)
			{
				...
				if (auto& root_move = my_thread->root_moves.moves[move_index]; move_number == 1 || value > alpha)
				{
					...
					assert((pi + 1)->pv);
					for (auto* z = (pi + 1)->pv; *z != no_move; ++z)root_move.pv.add(*z);
					if (move_number > 1 && my_thread == thread_pool.main())	static_cast<mainthread*>(my_thread)->best_move_changed += 1024;
					if (!bench_active && my_thread == thread_pool.main())acout() << print_pv(pos, alpha, beta, my_thread->active_pv, move_index) << std::endl;
				}
				else
			...
			if (value > best_score)
			{
				...
				if (value > alpha)
				{
					if (pv_node
						...
						easy_move.clear();
					best_move = move;
					if (pv_node && !root_node)copy_pv(pi->pv, move, (pi + 1)->pv);
					if (pv_node && value < beta)alpha = value;
					else
					{
						assert(value >= beta);
						break;
		...
		else
		{
			...
			if (constexpr auto fail_low_counter_move_bonus_min_depth_mult = 3;
				depth >= fail_low_counter_move_bonus_min_depth_mult * plies
				&& pi->position_value >= alpha - static_cast<int>(fail_low_counter_move_bonus_min_depth_margin))
				...
				&& pi->move_counter_values)
			{
				if (constexpr auto counter_move_bonus_min_depth = 18; depth < counter_move_bonus_min_depth * plies)
				{
					...
					if ((pi - 1)->move_counter_values)(pi - 1)->move_counter_values->update_plus(offset, bonus);
					if ((pi - 2)->move_counter_values)(pi - 2)->move_counter_values->update_plus(offset, bonus);
					if ((pi - 4)->move_counter_values)(pi - 4)->move_counter_values->update_plus(offset, bonus);
				}
			}
		}
		if (!pi->excluded_move)
		{
			...
			hash_entry->save(key64, value_to_hash(best_score, pi->ply),
				(best_score >= beta ? south_border : pv_node && best_move ? exact_value : north_border)
				+ pi->strong_threat, depth, best_move, pi->position_value, main_hash.age());
		}
		return best_score;
	}
	void calculate_egtb_mate_pv(position& pos, const int expected_score, principal_variation& pv)
	{
		...
		{
			pos.play_move(move);
			if (const auto number_pieces = pos.total_num_pieces(); number_pieces <= egtb::max_pieces_dtm)score = egtb::egtb_probe_dtm(pos);
	...
	int q_search(position& pos, int alpha, const int beta, const int depth)
	{
		...
		else
		{
			if (hash_entry && hash_entry->eval() != no_score)
			{
				...
				pi->strong_threat = hash_entry->threat();
				if (hash_value != no_score)
					if (hash_entry->bounds() & (hash_value > best_value ? south_border : north_border))	best_value = hash_value;
			...
			else
			{
				...
				{
					hash_entry = main_hash.replace(key64);
					hash_entry->save(key64, value_to_hash(best_value, pi->ply), south_border + pi->strong_threat, no_depth, no_move, pi->position_value, main_hash.age());
					return best_value;
				}
			}
			if (pv_node && best_value > alpha)alpha = best_value;
		...
		while ((move = movepick::pick_move(pos)) != no_move)
		{
				...
				&& !pos.advanced_pawn(move))
			{
				...
				if (constexpr auto qs_futility_basis_margin = 102;
					futility_basis + static_cast<int>(qs_futility_basis_margin) <= alpha)
				{
					...
					&& move < static_cast<uint32_t>(promotion_p))
				{
					...
					if (!gives_check)
					{
						...
						if (const auto stats_value = static_cast<int>(pos.thread_info()->evasion_history.value_at_offset(offset))
							...
							+ ((pi - 3)->move_counter_values ? static_cast<int>((pi - 3)->move_counter_values->value_at_offset(offset)) : sort_zero);
							stats_value < static_cast<int>(qs_stats_value_sortvalue))continue;
					}
					if (piece_type(moved_piece) != pt_king && !pos.see_test(move, see_0))continue;
				}
			}
			else
			{
				if (move < static_cast<uint32_t>(promotion_p) && !pos.see_test(move, see_0))continue;
			}
		skip_see_test:
			...
			if ((pi + 1)->captured_piece)
			{
				...
				if (const auto offset = move_value_stats::calculate_offset((pi + 1)->captured_piece, to_square(move));
					value > alpha)
			...
			if (value > best_value)
			{
				...
				if (value > alpha)
				{
					if (pv_node)copy_pv(pi->pv, move, (pi + 1)->pv);
	...
	void send_time_info()
	{
		...
		if (!bench_active && elapsed - previous_info_time >= 1000)
		{
			...
			acout() << "info time " << elapsed << " nodes " << nodes << " nps " << nps << " tbhits " << tb_hits << " hashfull " << main_hash.hash_full() << std::endl;
		}
		...
		const int depth, const uint32_t* quiet_moves, const int quiet_number)
	{
		...
		if (!pos.capture_or_promotion(move))
		{
			...
			if (cmh)ti->counter_moves.update(pi->moved_piece, to_square(pi->previous_move), static_cast<unsigned short>(move));
			if (cmh && fmh)ti->counter_followup_moves.update((pi - 1)->moved_piece, to_square((pi - 1)->previous_move), pi->moved_piece, to_square(pi->previous_move), move);
			if (constexpr auto update_stats_max_depth = 18; depth < update_stats_max_depth * plies)
			{
				...
				if (cmh)cmh->update_plus(offset, bonus);
				if (fmh)fmh->update_plus(offset, bonus);
				if (fmh2)fmh2->update_plus(offset, bonus);
				for (auto i = 0; i < quiet_number; ++i)
				{
					...
					if (cmh)cmh->update_minus(offset, bonus);
					if (fmh)fmh->update_minus(offset, bonus);
					if (fmh2)fmh2->update_minus(offset, bonus);
				}
			}
		}
	...
	void update_stats_quiet(const position& pos, const bool state_check, const int depth, const uint32_t* quiet_moves,
		const int quiet_number)
	{
		const auto* pi = pos.info();
		auto* cmh = pi->move_counter_values;
		auto* fmh = (pi - 1)->move_counter_values;
		...
		if (constexpr auto update_stats_quiet_max_depth = 18; depth < update_stats_quiet_max_depth * plies)
		{
			...
			for (auto i = 0; i < quiet_number; ++i)
			{
				const auto offset = move_value_stats::calculate_offset(pos.moved_piece(quiet_moves[i]),
					to_square(quiet_moves[i]));
				history.update_minus(offset, hist_bonus);
				if (cmh)cmh->update_minus(offset, bonus);
				if (fmh)fmh->update_minus(offset, bonus);
				if (fmh2)fmh2->update_minus(offset, bonus);
	...
	{
		assert(val != no_score);
		return val >= longest_mate_score ? val + ply : val <= longest_mated_score ? val - ply : val;
	}
	int value_from_hash(const int val, const int ply)
	{
		return val == no_score ? no_score : val >= longest_mate_score ? val - ply : val <= longest_mated_score ? val + ply : val;
	}
}
void mainthread::begin_search()
{
	...
	thread_pool.fifty_move_distance = std::min(50, std::max(thread_pool.fifty_move_distance,
		root_position->fifty_move_counter() / 2 + 5));
	thread_pool.piece_contempt = uci_contempt;
	if (thread_pool.piece_contempt)
	{
		if (thread_pool.analysis_mode)thread_pool.contempt_color = white;
		...
		if (const auto v2 = evaluate::eval(*root_position); abs(v1) < win_score && abs(v2) < win_score)	thread_pool.root_contempt_value = v2 - v1;
		else thread_pool.root_contempt_value = score_0;
	}
	...
	if (!search::param.ponder)main_hash.new_age();
	tb_root_in_tb = false;
...
NO_ANALYSIS:
	...
	{
		if (root_moves[0].depth == main_thread_inc)root_moves[0].depth = 99 * main_thread_inc;
		search::signals.stop_if_ponder_hit = true;
		wait(search::signals.stop_analyzing);
	}
	search::signals.stop_analyzing = true;
	for (auto i = 1; i < thread_pool.active_thread_count; ++i)thread_pool.threads[i]->wait_for_search_to_end();
	...
	if (best_thread != this || search::signals.stop_if_ponder_hit)best_thread->root_moves[0].depth = root_moves[0].depth;
	if (!bench_active)
	{
...
void thread::begin_search()
{
	...
	(pi - 1)->move_number = 0;
	(pi - 4)->move_counter_values = (pi - 3)->move_counter_values = (pi - 2)->move_counter_values = (pi - 1)->move_counter_values = pi->move_counter_values = nullptr;
	...
	if (main_thread)
	{
		...
		for (auto i = 1; i <= max_ply; i++)	(pi + i)->pawn_key = 0;
	}
	...
	{
		if (const auto* hash_entry = main_hash.probe(root_position->key());
			hash_entry && hash_entry->bounds() == exact_value)
		{
	...
	if (main_thread)main_thread->quick_move_allow = true;
	...
	while (++search_iteration < 100)
	{
		...
		if (main_thread)
		{
			if (search::param.depth && search_iteration - 1 >= search::param.depth)	search::signals.stop_analyzing = true;
		}
		if (search::signals.stop_analyzing)break;
		if (main_thread)
		{
		...
		for (auto i = 0; i < root_moves.move_number; i++)root_moves[i].previous_score = root_moves[i].score;
		for (active_pv = 0; active_pv < thread_pool.multi_pv && !search::signals.stop_analyzing; ++active_pv)
		{
			...
			while (true)
			{
				...
				if (alpha < -value_pawn_mult * value_pawn)alpha = -max_score;
				if (beta > value_pawn_mult * value_pawn)beta = max_score;
					...
					&& time_control.elapsed() > time_control.optimum() * time_control_optimum_mult_1 / 1024)
				{
					...
					else if (constexpr auto time_control_optimum_mult_2 = 420; time_control.elapsed() > time_control.optimum() * time_control_optimum_mult_2 / 1024)
					{
						...
						if (const auto unstable_factor = 1024 + main_thread->best_move_changed; time_control.elapsed() > time_control.optimum() * unstable_factor / 1024
							* improvement_factor / 1024)
							fail_high_resolve = false;
					}
				}
				if (best_value <= alpha)
				...
				else if (constexpr auto best_value_vp_mult = 8;
					best_value >= beta && (fail_high_resolve || best_value >= value_pawn * best_value_vp_mult))
				{
					alpha = (alpha + beta) / 2;
		...
		if (!thread_pool.analysis_mode)
		{
			...
			{
				const auto improvement_factor = std::max(420,
					std::min(improvement_factor_min_base, improvement_factor_max_base
						+ improvement_factor_max_mult
						* main_thread->failed_low - improvement_factor_bv_mult
						* (best_value - main_thread->previous_root_score)));
				...
				{
					if (search::param.ponder)search::signals.stop_if_ponder_hit = true;
					else search::signals.stop_analyzing = true;
				}
			}
			if (root_moves[0].pv.size() >= 3)search::easy_move.refresh_pv(*root_position, root_moves[0].pv);
			else search::easy_move.clear();
		}
	}
	if (!main_thread)return;
	if (search::easy_move.third_move_stable < 6 || main_thread->quick_move_played)search::easy_move.clear();
}
void filter_root_moves(position& pos)
{
	...
	if (tb_number < pos.total_num_pieces() || pos.castling_possible(all))return;
	tb_root_in_tb = egtb::egtb_probe_dtz(pos);
	if (tb_root_in_tb)tb_number = 0;
	if (tb_root_in_tb && !egtb::use_rule50)
		tb_score = tb_score > draw_score
...
bool rootmove::ponder_move_from_hash(position& pos)
{
	...
	{
		if (const auto move = hash_entry->move(); legal_moves_list_contains_move(pos, move))pv.add(move);
	}
...
std::string print_pv(const position& pos, const int alpha, const int beta, const int active_pv, const int active_move)
{
	...
	for (auto i = 0; i < multi_pv; ++i)
	{
		...
		for (sel_depth = 0; sel_depth < max_ply; sel_depth++)
			if ((pi + sel_depth)->pawn_key == 0)break;
		ss << "info time " << elapsed
			<< " nodes " << visited_nodes
			<< " nps " << visited_nodes / elapsed * 1000
			<< " tbhits " << tb_hits;
		if (hash_full)ss << " hashfull " << hash_full;
		ss << " depth " << iterate
			<< " seldepth " << sel_depth
			<< " multipv " << i + 1
			<< " score " << score_cp(score);
		if (!tb && i == active_pv)	ss << (score >= beta ? " lowerbound" : score <= alpha ? " upperbound" : "");
		ss << " pv";
		auto pv_length = root_move.pv.size();
		if (pv_length > iterate)pv_length = std::max(iterate, pv_length - 4);
		for (auto n = 0; n < pv_length; n++)ss << " " << util::move_to_string(root_move.pv[n], pos);
	}
	return ss.str();
}
void rootmove::pv_from_hash(position& pos)
{
	...
	while (true)
	{
		...
		if (!move || !legal_moves_list_contains_move(pos, move))break;
		pv.add(move);
	}
	for (auto i = pv.size(); i > 0;)pos.take_move_back(pv[--i]);
}
std::string score_cp(const int score)
{
	...
	{
		if (uci_mcts == true)ss << "cp " << score;
		else ss << "cp " << score / 3;
	}
	else ss << "mate " << (score > 0 ? mate_score - score + 1 : -mate_score - score) / 2;
	return ss.str();
}
