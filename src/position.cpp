...
square position::calculate_threat() const
{
	...
	{
	case w_pawn:
		if (const auto b = pawnattack[white][to] & pieces_excluded(black, pt_pawn)) return lsb(b);
		break;
	case b_pawn:
		if (const auto b = pawnattack[black][to] & pieces_excluded(white, pt_pawn)) return msb(b);
		break;
	case w_knight:
		if (const auto b = empty_attack[pt_knight][to] & pieces(black, pt_rook, pt_queen)) return lsb(b);
		break;
	case b_knight:
		if (const auto b = empty_attack[pt_knight][to] & pieces(white, pt_rook, pt_queen)) return msb(b);
		break;
	case w_bishop:
		if (const auto b = attack_bishop_bb(to, pieces()) & pieces(black, pt_rook, pt_queen)) return lsb(b);
		break;
	case b_bishop:
		if (const auto b = attack_bishop_bb(to, pieces()) & pieces(white, pt_rook, pt_queen)) return msb(b);
		break;
	case w_rook:
		if (const auto b = attack_rook_bb(to, pieces()) & pieces(black, pt_queen)) return lsb(b);
		break;
	case b_rook:
		if (const auto b = attack_rook_bb(to, pieces()) & pieces(white, pt_queen)) return msb(b);
		break;
	case no_piece: break;
	case w_king: break;
	case w_queen: break;
...
bool position::give_check(const uint32_t move) const
{
	...
	const auto square_k = king(~on_move_);
	if (pos_info_->check_squares[piece_type(piece_on_square(from))] & to) return true;
	if (pos_info_->x_ray[~on_move_] & from && !aligned(from, to, square_k)) return true;
	if (move < static_cast<uint32_t>(castle_move)) return false;
	if (move >= static_cast<uint32_t>(promotion_p)) return attack_bb(promotion_piece(move), to, pieces() ^ from) & square_k;
	if (move < static_cast<uint32_t>(enpassant))
	{
		...
		return empty_attack[pt_rook][to_r] & square_k && attack_rook_bb(to_r, pieces() ^ from ^ from_r | to_r | to) & square_k;
	}
	{
		const auto ep_square = make_square(file_of(to), rank_of(from));
		const auto b = pieces() ^ from ^ ep_square | to;
		return (attack_rook_bb(square_k, b) & pieces(on_move_, pt_queen, pt_rook)) | (attack_bishop_bb(square_k, b) & pieces(on_move_, pt_queen, pt_bishop));
	}
}
void position::init()
{
...
bool position::is_draw() const
{
	...
	{
		if (pos_info_->draw50_moves == 100) return !pos_info_->in_check || at_least_one_legal_move(*this);
		return true;
	}
	...
	do
	{
		if (stp->key == pos_info_->key)	return true;
		stp -= 2;
		n -= 2;
	} while (n >= 0);
	return false;
...
bool position::legal_move(const uint32_t move) const
{
	...
	if (move_type(move) == enpassant)
	{
		...
		return !(attack_rook_bb(square_k, occupied) & pieces(~me, pt_queen, pt_rook)) && !(attack_bishop_bb(square_k, occupied) & pieces(~me, pt_queen, pt_bishop));
	}
...
void position::play_move(const uint32_t move, const bool gives_check)
{
	...
	if (capture_piece)
	{
		...
		else pos_info_->non_pawn_material[you] -= material_value[capture_piece];
	...
	{
		if ((static_cast<int>(to) ^ static_cast<int>(from)) == 16 && attack_from<pt_pawn>(to - pawn_ahead(me), me) & pieces(you, pt_pawn))
		{
		...
		else if (move >= static_cast<uint32_t>(promotion_p))
		{
			...
			pos_info_->material_key ^= zobrist::psq[promotion][piece_number_[promotion] - 1] ^ zobrist::psq[piece][piece_number_[piece]];
...
void position::play_null_move()
{
	++nodes_;
	auto key = pos_info_->key ^ zobrist::on_move;
	if (pos_info_->enpassant_square != no_square)key ^= zobrist::enpassant[file_of(pos_info_->enpassant_square)];
	main_hash.prefetch_entry(key);
	std::memcpy(pos_info_ + 1, pos_info_, offsetof(position_info, key));
	pos_info_++;
	pos_info_->key = key;
...
bool position::see_test(const uint32_t move, const int limit) const
{
	...
	do
	{
		...
		if ((bb = my_attackers & pieces(pt_pawn)))capture_piece = pt_pawn;
		else if ((bb = my_attackers & pieces(pt_knight)))capture_piece = pt_knight;
		else if ((bb = my_attackers & pieces(pt_bishop)))capture_piece = pt_bishop;
		else if ((bb = my_attackers & pieces(pt_rook)))capture_piece = pt_rook;
		else if ((bb = my_attackers & pieces(pt_queen)))capture_piece = pt_queen;
		else return attackers & pieces(me);
		value += see_value[capture_piece];
		if (value < 0) return false;
		occupied ^= bb & -bb;
		if (!(capture_piece & 1))attackers |= attack_bishop_bb(to, occupied) & (pieces(pt_bishop) | pieces(pt_queen));
		if (capture_piece >= pt_rook)attackers |= attack_rook_bb(to, occupied) & (pieces(pt_rook) | pieces(pt_queen));
		attackers &= occupied;
...
void position::set_castling_possibilities(const side color, const square from_r)
{
	...
	uint64_t pad = 0;
	for (auto sq = std::min(from_r, to_r); sq <= std::max(from_r, to_r); ++sq)pad |= sq;
	for (auto sq = std::min(from_k, to_k); sq <= std::max(from_k, to_k); ++sq)pad |= sq;
	castle_path_[castle] = pad & ~(square_bb[from_k] | square_bb[from_r]);
	if (from_k != relative_square(color, e1))chess960_ = true;
	if (from_k < from_r && from_r != relative_square(color, h1))chess960_ = true;
	if (from_k >= from_r && from_r != relative_square(color, a1))chess960_ = true;
}
void position::set_position_info(position_info* si) const
{
	...
	if (si->enpassant_square != no_square)si->key ^= zobrist::enpassant[file_of(si->enpassant_square)];
	if (on_move_ == black)si->key ^= zobrist::on_move;
...
position& position::set(const std::string& fen_str, const bool is_chess960, thread* th)
{
	...
	{
		if (isdigit(token))	sq += static_cast<square>(token - '0');
		else if (token == '/') sq -= static_cast<square>(16);
	...
	while (ss >> token && !isspace(token))
	{
		...
		if (token == 'K')for (rsq = relative_square(color, h1); piece_on_square(rsq) != rook; --rsq) {}
		else if (token == 'Q')for (rsq = relative_square(color, a1); piece_on_square(rsq) != rook; ++rsq) {}
		else if (token >= 'A' && token <= 'H')rsq = make_square(static_cast<file>(token - 'A'), relative_rank(color, rank_1));
		else continue;
		set_castling_possibilities(color, rsq);
	}
	if (uint8_t f = 0; ss >> f && (f >= 'a' && f <= 'h') && (ss >> r && (r == '3' || r == '6')))
	{
		pos_info_->enpassant_square = make_square(static_cast<file>(f - 'a'), static_cast<rank>(r - '1'));
		if (!(attack_to(pos_info_->enpassant_square) & pieces(on_move_, pt_pawn)))pos_info_->enpassant_square = no_square;
	}
	else pos_info_->enpassant_square = no_square;
...
void position::take_move_back(const uint32_t move)
{
	...
	if (move < static_cast<uint32_t>(castle_move))
	{
		...
		if (pos_info_->captured_piece)	move_piece(~me, pos_info_->captured_piece, to);
	}
	else
	{
...
bool position::valid_move(const uint32_t move) const
{
	const auto me = on_move_;
	const auto from = from_square(move);
	if (!(pieces(me) & from)) return false;
	const auto to = to_square(move);
	...
	if (move >= static_cast<uint32_t>(castle_move))
	{
		...
		else if (move < static_cast<uint32_t>(enpassant)) return legal_move_list_contains_castle(*this, move);
		else return legal_moves_list_contains_move(*this, move);
	}
	else
	{
	}
	...
	else if (!(attack_from(piece, from) & to)) return false;
	if (is_in_check())
	{
		if (piece != pt_king)
		{
			if (more_than_one(is_in_check())) return false;
			if (!((get_between(lsb(is_in_check()), king(me)) | is_in_check()) & to)) return false;
		}
		else if (attack_to(to, pieces() ^ from) & pieces(~me)) return false;
	}
	return true;
}
