...
#include "bitop.h"
#include "fire.h"
enum hashflags : uint8_t { no_limit, threat_white = 1, threat_black = 2, in_use = 4, south_border = 64, north_border = 128, exact_value = north_border | south_border };
...
class hash
{
	...
	struct bucket { main_hash_entry entry[bucket_size]; char padding[2]; };
...
public:
	...
	void clear() const;
	[[nodiscard]] main_hash_entry* entry(const uint64_t key) const { return reinterpret_cast<main_hash_entry*>(reinterpret_cast<char*>(hash_mem_) + (key & bucket_mask_)); }
