...
static constexpr uint8_t piece_type(const ptype piece) { return static_cast<uint8_t>(piece & 7); }
...
inline int position::castling_possible(const side color) const { return pos_info_->castle_possibilities & (white_short | white_long) << (2 * color); }
inline cmhinfo* position::cmh_info() const { return cmh_info_; }
inline bool position::different_color_bishops() const { return piece_number_[w_bishop] == 1 && piece_number_[b_bishop] == 1 && different_color(piece_square(white, pt_bishop), piece_square(black, pt_bishop)); }
...
inline void position::increase_tb_hits() { ++tb_hits_; }
inline bool position::is_capture_move(const uint32_t move) const { assert(is_ok(move)); return !empty_square(to_square(move)) && move_type(move) != castle_move || move_type(move) == enpassant; }
...
inline bool position::is_passed_pawn(const side color, const square sq) const { return !(pieces(~color, pt_pawn) & passedpawn_mask(color, sq)); }
...
inline bool position::material_or_castle_changed() const { return pos_info_->material_key != (pos_info_ - 1)->material_key || pos_info_->castle_possibilities != (pos_info_ - 1)->castle_possibilities; }
...
inline int position::number(const side color, const uint8_t piece) const { return piece_number_[make_piece(color, piece)]; }
...
inline const square* position::piece_list(const side color, const uint8_t piece) const { return piece_list_[make_piece(color, piece)]; }
inline ptype position::piece_on_square(const square sq) const { return board_[sq]; }
inline square position::piece_square(const side color, const uint8_t piece) const { assert(piece_number_[make_piece(color, piece)] == 1); return piece_list_[make_piece(color, piece)][0]; }
inline uint64_t position::pieces_excluded(const side color, const uint8_t piece) const { return color_bb_[color] ^ piece_bb_[make_piece(color, piece)]; }
inline uint64_t position::pieces() const { return piece_bb_[all_pieces]; }
inline uint64_t position::pieces(const uint8_t piece) const { return piece_bb_[make_piece(white, piece)] | piece_bb_[make_piece(black, piece)]; }
inline uint64_t position::pieces(const uint8_t piece1, const uint8_t piece2) const { return pieces(piece1) | pieces(piece2); }
inline uint64_t position::pieces(const side color) const { return color_bb_[color]; }
inline uint64_t position::pieces(const side color, const uint8_t piece) const { return piece_bb_[make_piece(color, piece)]; }
inline uint64_t position::pieces(const side color, const uint8_t piece1, const uint8_t piece2) const { return pieces(color, piece1) | pieces(color, piece2); }
inline uint64_t position::pieces(const side color, const uint8_t piece1, const uint8_t piece2, const uint8_t piece3) const { return pieces(color, piece1) | pieces(color, piece2) | pieces(color, piece3); }
...
inline uint64_t position::tb_hits() const { return tb_hits_; }
