...
struct search_signals { std::atomic_bool stop_analyzing, stop_if_ponder_hit; };
namespace search {
	...
	{
		void clear() { key_after_two_moves = 0; third_move_stable = 0; pv[0] = pv[1] = pv[2] = no_move; }
		[[nodiscard]] uint32_t expected_move(const uint64_t key) const { return key_after_two_moves == key ? pv[2] : no_move; }
	...
	inline int futility_margin_ext(const int d) { return futility_margin_ext_base + futility_margin_ext_mult * static_cast<int>(static_cast<uint32_t>(d) / plies); }
	...
	inline int late_move_number(const int d, const bool progress) { return late_move_number_values[progress][static_cast<uint32_t>(d) / (plies / 2)]; }
	...
	struct rootmove_mc
	{
		...
		bool operator==(const uint32_t& m) const { return pv[0] == m; }
		bool operator<(const rootmove_mc& m) const { return m.score != score ? m.score < score : m.previous_score < previous_score; }
		int score = -max_score;
...
{
	rootmove() = default;
	explicit rootmove(const uint32_t move) { pv.move_number = 1; pv.moves[0] = move; }
