...
main_hash_entry* hash::probe(const uint64_t key) const
{
	...
	for (auto i = 0; i < bucket_size; ++i)
		...
		{
			if ((hash_entry[i].flags_ & age_mask) != age_)hash_entry[i].flags_ = static_cast<uint8_t>(age_ + (hash_entry[i].flags_ & flags_mask));
			return &hash_entry[i];
		}
	return nullptr;
}
...
main_hash_entry* hash::replace(const uint64_t key) const
{
	...
	for (auto i = 1; i < bucket_size; ++i)
		if (replacement->depth_ - (age_ - (replacement->flags_ & age_mask) & age_mask) > hash_entry[i].depth_
			- (age_ - (hash_entry[i].flags_ & age_mask) & age_mask))replacement = &hash_entry[i];
	return replacement;
}
